...
async function loadGames() {
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            html += `
                <tr>
                    <td><img src="${data.iconUrl || ''}" class="table-img" alt="icon" style="width:40px;height:40px;border-radius:8px;"></td>
...
async function handleSaveGame(e) {
    ...
    const iconUrl = document.getElementById("gameIconUrl").value.trim();
    const order = parseInt(document.getElementById("gameOrder").value) || 0;
    const isActive = document.getElementById("gameStatus").checked;

    if (!name || !category) { alert("Nama dan Kategori wajib diisi!"); return; }

    btnSubmit.classList.add("btn-loading");
    btnSubmit.disabled = true;

    try {
        const gameData = { name, category, iconUrl, order, isActive, updatedAt: new Date().toISOString() };
        if (isEditing && editId) {
...
window.deleteGameData = async function(id) {
    if (confirm("Hapus game ini?")) {
        try { await deleteDoc(doc(db, "games", id)); loadGames(); }
