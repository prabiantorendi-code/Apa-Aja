...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const gameIconFile = document.getElementById("gameIconFile");
    ...
    if (gameIconFile) gameIconFile.addEventListener("change", handleFileSelect);
...
async function loadGames() {
    ...
    try {
        ...
        tbody.innerHTML = html || `<tr><td colspan="6" style="text-align: center; padding: 20px; color: var(--text-muted);">Belum ada data game</td></tr>`;
    } catch (error) {
...
async function handleSaveGame(e) {
    ...
    try {
        ...
        // Upload gambar ke Storage jika ada file baru
        if (selectedFile) {
            const storageRef = ref(storage, `games/${Date.now()}_${selectedFile.name}`);
        ...
        if (isEditing && editId) {
            ...
            if (!selectedFile && !iconUrl) {
                alert("Wajib upload icon untuk game baru!");
